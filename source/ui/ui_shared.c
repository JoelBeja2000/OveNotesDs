...
void drawModalButtonAt(int x0, int x1, int y0, int y1, const char* text, bool selected) {
    uint16_t bg = selected ? blendRGB555_int(app_theme_color, RGB15(2, 2, 3), 16) : RGB15(2, 2, 3); // note: 16 instead of 8 to match the blendRGB555_int usage or RGB15(4,4,5) blend
    bg = selected ? blendRGB555_int(app_theme_color, RGB15(4, 4, 5), 8) : RGB15(2, 2, 3);
