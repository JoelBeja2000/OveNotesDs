...
fn add_cors_headers<R: Read>(response: Response<R>) -> Response<R> {
    response
        ...
        .with_header(tiny_http::Header::from_bytes(&b"Access-Control-Allow-Methods"[..], &b"GET, POST, OPTIONS, DELETE"[..]).unwrap())
        ...
        for mut request in server.incoming_requests() {
            ...
            if url == "/api/pairing-code" && method == Method::Get {
                ...
                if let Ok(mut html) = fs::read_to_string("dist/index.html") {
                    ...
                    let injection_token = "let pairingCodeData = { code: '------', ip: '...', port: '...' };";
                    let injected = format!(r#"let pairingCodeData = {{ code: '{}', ip: '{}', port: {} }};"#, code, ip, port);
                    ...
                    for (i, (name, time)) in notes.iter().enumerate() {
                        if i > 0 { cached_notes_json.push(','); }
                        cached_notes_json.push_str(&format!(r#"{{"name":"{}","time":{}}}"#, name, time));
                    }
                    ...
                    let response = add_cors_headers(
                        Response::from_string("index.html not found")
                            .with_status_code(404)
                    );
                    let _ = request.respond(response);
                }
