...
GAME_TITLE	?= OveNotesDS
GAME_SUBTITLE	?= Wifi FTP Tool
GAME_AUTHOR	?= Joel
...
SOURCEDIRS	?= source
INCLUDEDIRS	?= include source source/ui source/ui/views source/ui/modals source/ui/widgets source/ui/forms source/systems source/assets source/vendor
GFXDIRS		?=
BINDIRS		?=
AUDIODIRS	?=
